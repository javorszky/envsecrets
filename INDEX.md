...
| `(c *Client) EnsureVault(_ context.Context) (bool, error)` | First-time: validates and prompts for OAT (no echo on TTY, pipe-safe), pre-creates the config file at `0600` to prevent the SDK's `os.Create` at `0666`, registers device with KSM via `initManager`, runs a probe; on probe failure removes the partial config so the next run re-prompts. Subsequent: best-effort `chmod` on existing file (0600) and directory (0700) to tighten permissions from older or manually-created configs, then loads config and runs probe. Returns `(true, nil)` on first creation, `(false, nil)` if already valid. All SDK writes go through `secureStorage` which keeps file at `0600`. |
